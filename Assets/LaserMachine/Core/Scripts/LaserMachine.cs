...
namespace Lightbug.LaserMachine
{
    ...
    //            element.lineRenderer.gameObject.AddComponent<BoxCollider>();
    //            element.lineRenderer.gameObject.GetComponent<BoxCollider>().size = new Vector3(1, 1.000008f, 26.41392f);
                
    //            element.lineRenderer.gameObject.GetComponent<BoxCollider>().center = new Vector3(0, -9.536745e-07f, 12.49999f);
    //            element.lineRenderer.gameObject.GetComponent<BoxCollider>().isTrigger = true;
    //            element.lineRenderer.gameObject.AddComponent<DestroyHuman>();
				//element.lineRenderer.gameObject.AddComponent<ColliderDeactivator>();
				//element.lineRenderer.gameObject.GetComponent<DestroyHuman>().pieceHuman = pieceHuman;
    //        }
                ...
                if (m_active)
                {
                    ...
                    if (m_currentProperties.m_physicsType == LaserProperties.PhysicsType.Physics3D)
                    {
                        ...
                        if (hitInfo3D.collider)
                        {
                            ...
                            if(hitInfo3D.collider.gameObject.tag == "flying_human")
                            {
                                Destroy(hitInfo3D.collider.gameObject);
                                GameObject pieces = Instantiate(pieceHuman, hitInfo3D.collider.gameObject.transform.position, hitInfo3D.collider.gameObject.transform.rotation);
